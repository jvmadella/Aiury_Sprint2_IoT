...
-- Schema: WKSP_ALERTSWORKSPACE  Date: Sun Nov 02 07:18:31 2025 
--
...
BEGIN
  ORDS.ENABLE_SCHEMA(
      ...
      p_url_mapping_pattern => 'alertsworkspace',
      p_auto_rest_auth      => TRUE);

  ORDS.DEFINE_MODULE(
      p_module_name    => 'Alerts API',
      p_base_path      => '/alerts/',
      p_items_per_page => 25,
      p_status         => 'PUBLISHED',
      p_comments       => NULL);
  ...
  ORDS.DEFINE_HANDLER(
      ...
      p_source         => 
'BEGIN
  INSERT INTO ALERTS(USER_ID, MESSAGE, LAT, LNG, RISK)
  VALUES(:USER_ID, :MESSAGE, :LAT, :LNG, :RISK);
  COMMIT;
END;');
...
exception
 ...
 when others then 
  ...
  ORDS.CREATE_ROLE(
      p_role_name=> 'oracle.dbtools.role.autorest.WKSP_ALERTSWORKSPACE');
  ORDS.CREATE_ROLE(
      p_role_name=> 'oracle.dbtools.role.autorest.WKSP_ALERTSWORKSPACE.ALERTS');
  ORDS.CREATE_ROLE(
      p_role_name=> 'oracle.dbtools.role.autorest.any.WKSP_ALERTSWORKSPACE');
  l_roles(1) := 'oracle.dbtools.autorest.any.schema';
  l_roles(2) := 'oracle.dbtools.role.autorest.WKSP_ALERTSWORKSPACE';
  l_patterns(1) := '/metadata-catalog/*';

  ORDS.DEFINE_PRIVILEGE(
      p_privilege_name => 'oracle.dbtools.autorest.privilege.WKSP_ALERTSWORKSPACE',
      p_roles          => l_roles,
      p_patterns       => l_patterns,
      p_modules        => l_modules,
      p_label          => 'WKSP_ALERTSWORKSPACE metadata-catalog access',
      p_description    => 'Provides access to the metadata catalog of the objects in the WKSP_ALERTSWORKSPACE schema.',
      p_comments       => NULL); 
  ...
  l_roles(1) := 'oracle.dbtools.autorest.any.schema';
  l_roles(2) := 'oracle.dbtools.role.autorest.WKSP_ALERTSWORKSPACE.ALERTS';
  l_roles(3) := 'oracle.dbtools.role.autorest.any.WKSP_ALERTSWORKSPACE';

  ORDS.DEFINE_PRIVILEGE(
      p_privilege_name => 'oracle.dbtools.autorest.privilege.WKSP_ALERTSWORKSPACE.ALERTS',
      p_roles          => l_roles,
      p_patterns       => l_patterns,
      p_modules        => l_modules,
      p_label          => 'WKSP_ALERTSWORKSPACE.ALERTS access',
      p_description    => 'Provides access to the ALERTS TABLE in the WKSP_ALERTSWORKSPACE schema.',
      p_comments       => NULL); 
